...
```mermaid
flowchart TB
    subgraph Access["Access & Navigation"]
...
```mermaid
flowchart LR
    A["Click 'Choose File'"] --> B["Select MP4 File"]
    B --> C["Name Presentation"]
    C --> D["Select Destination"]
    D --> E["Create Presentation"]
    E --> F["Queue & Process"]
    F --> G["Unlock for Viewing"]
```
...
```mermaid
flowchart LR
    A["Click Create"] --> B["Select Channel Option"]
    B --> C["Name Channel"]
    C --> D["Create Channel"]
    D --> E["Share Channel Link"]
```
