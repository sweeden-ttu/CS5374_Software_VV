...
def get_langsmith_callbacks():
    ...
    tracing = os.getenv("LANGCHAIN_TRACING_V2", "").strip().lower() in {"1", "true", "yes", "y"}
    if not tracing:
        return []
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "Set LANGCHAIN_API_KEY to your LangSmith API key (and optionally LANGCHAIN_PROJECT). "
        )

    # Lazy import so users without langsmith installed can still run without tracing.
    from langchain.callbacks.tracers import LangChainTracer

    project = os.getenv("LANGCHAIN_PROJECT", "LangGraph_Demo2").strip() or "LangGraph_Demo2"
